...
```{r}
summary(tornadoes$DAMAGE_PROPERTY)
```

`DAMAGE_PROPERTY` is a character field. This should be converted to a numeric field. You can use any of the cleaning functions at any time. These cleaning functions have a prefix that identifies the dataset for which they're built. In this instance, I'll use `details_damages`.
...
```{r}
summary(dp$DAMAGE_PROPERTY)
```

With the tornado costing $8M that puts it between the 25th and 50th percentile. That's a lot of money but nowhere close to the most expensive tornado for 2015.
